...
class AVL():
    ...
    def insert(self,node,data):
        ...
        if b > 1 and data < node.left.data: #leftleft
            return self.rRotate(node)
        if b > 1 and data >= node.left.data: #leftright
            node.left = self.lRotate(node.left)
            return self.rRotate(node)
        if b < -1 and data >= node.right.data: #rightright
            return self.lRotate(node)
        if b < -1 and data < node.right.data: #rightleft
